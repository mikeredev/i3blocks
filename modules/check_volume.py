...
def is_microphone_enabled():
    ...
    awk_cmd = ["awk", "/\\* index/{getline; print; for(i=1;i<=10;i++) {getline; print}}"]
    grep_cmd = ["grep", "-i", "muted"]
    cut_cmd = ["cut", "-d:", "-f2"]
    awk2_cmd = ["awk", "{$1=$1};1"]

    pacmd_output = subprocess.Popen(pacmd_cmd, stdout=subprocess.PIPE)
    awk_output = subprocess.Popen(awk_cmd, stdin=pacmd_output.stdout, stdout=subprocess.PIPE)
    grep_output = subprocess.Popen(grep_cmd, stdin=awk_output.stdout, stdout=subprocess.PIPE)
    cut_output = subprocess.Popen(cut_cmd, stdin=grep_output.stdout, stdout=subprocess.PIPE)
    awk2_output = subprocess.Popen(awk2_cmd, stdin=cut_output.stdout, stdout=subprocess.PIPE)
...
def is_webcam_enabled():
    ...
    grep_output = subprocess.Popen(grep_cmd, stdin=lsof_output.stdout, stdout=subprocess.PIPE)
...
def check(warning, critical):
    ...
    if check_bluetooth.lower() == "true":
        try:
            if is_bluetooth_enabled():
                bluetooth_icon = "<span font='FontAwesome' foreground='#FFFFFF'>\uf294</span>"
            else:
                bluetooth_icon="<span font='FontAwesome' foreground='#666666'>\uf294</span>"
        except:
            bluetooth_icon = "<span font='FontAwesome' foreground='#dd2222'>\uf294</span>"
            
    if check_webcam.lower() == "true":
        try:
            if is_webcam_enabled():
                webcam_icon = "<span font='FontAwesome' foreground='#FFFFFF'>\uf03d</span>"
            else:
                webcam_icon="<span font='FontAwesome' foreground='#666666'>\uf03d</span>"
        except:
    ...
    output = f"{volume}% <span font='FontAwesome' foreground='{color}'>{audio_icon}</span>"
    if check_mic.lower() == "true":
        output += f" {mic_icon}"
    if check_bluetooth.lower()=="true":
